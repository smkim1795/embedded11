...
void fb_write(char* picData, int picWidth, int picHeight) {
    int coor_y = 0;
    int coor_x = 0;
    int targetHeight = fbHeight;  // 변경된 높이
    int targetWidth = fbWidth;    // 변경된 너비

    // 비율에 맞춰 이미지 크기 조정
    for (coor_y = 0; coor_y < targetHeight; coor_y++) {
        int bmpYOffset = (coor_y * picHeight / targetHeight) * picWidth * 3;  // 변경된 Y 오프셋
        int bmpXOffset = 0;
        for (coor_x = 0; coor_x < targetWidth; coor_x++) {
            int bmpX = coor_x * picWidth / targetWidth;  // 변경된 X 오프셋
            // BMP: B-G-R로 인코딩 됨, FB: 0-R-G-B로 인코딩 됨.
            pfbmap[coor_y * fbWidth + (fbWidth - coor_x) + currentEmptyBufferPos] = 
                ((unsigned long)(picData[bmpYOffset + bmpX * 3 + 2]) << 16) +
                ((unsigned long)(picData[bmpYOffset + bmpX * 3 + 1]) << 8) +
                ((unsigned long)(picData[bmpYOffset + bmpX * 3 + 0]));
        }
    }
#ifdef ENABLED_DOUBLE_BUFFERING
    fb_doubleBufSwap();
