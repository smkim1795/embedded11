...
{ // 스레드 1 : 미로 게임
    while (1)
    {
        if (stage == 1)
        {
            ...
            while (stage1_end == 0)
            {
                read_bmp("red.bmp", &data, &cols, &rows); // 미로 게임에서 사용되는 선 긋는 용 bmp 출력
                ...
                read_bmp("red.bmp", &data, &cols, &rows); // 이동하는 aim의 새로운 좌표를 업데이트하여 다시 출력
                fb_write_c(data, cols, rows, new_x, new_y);
...
int main(void)
{
    ...
    read_bmp("output.bmp", &data, &cols, &rows); // 게임 시작 초기 화면 :: STRAT PAGE
    fb_write(data, cols, rows);
